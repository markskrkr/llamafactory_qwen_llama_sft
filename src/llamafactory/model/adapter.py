...
try:
    ...
    RANDLORA_AVAILABLE = True
except ImportError:
    RANDLORA_AVAILABLE = False

try:
    from peft import VBLoRAConfig
    VBLORA_AVAILABLE = True
except ImportError:
    VBLORA_AVAILABLE = False
...
def _setup_lora_tuning(
    ...
    if is_trainable:
        ...
        elif finetuning_args.use_randlora:
            logger.info_rank0("Fine-tuning method: RandLoRA")
        elif finetuning_args.use_vblora:
            logger.info_rank0("Fine-tuning method: VB-LoRA")
        else:
    ...
    if is_trainable and adapter_to_resume is None:  # create new lora weights while training
        ...
        if model_args.use_kt:
            ...
            if finetuning_args.use_randlora:
                if not RANDLORA_AVAILABLE:
                    raise ValueError("RandLoRA is not available. Please upgrade PEFT to the latest version.")

                logger.info_rank0("Using RandLoRA initialization.")
                peft_config = RandLoraConfig(
                    r=finetuning_args.lora_rank,
                    target_modules=target_modules,
                    randlora_alpha=finetuning_args.lora_alpha,
                    randlora_dropout=finetuning_args.lora_dropout,
                    projection_prng_key=finetuning_args.randlora_prng_key,
                    modules_to_save=finetuning_args.additional_target,
                    task_type=TaskType.CAUSAL_LM,
                    inference_mode=False,
                )
            elif finetuning_args.use_vblora:
                if not VBLORA_AVAILABLE:
                    raise ValueError("VB-LoRA is not available. Please upgrade PEFT to the latest version.")

                logger.info_rank0("Using VB-LoRA initialization.")
                peft_config = VBLoRAConfig(
                    r=finetuning_args.lora_rank,
                    num_vectors=finetuning_args.vblora_num_vectors,
                    vector_length=finetuning_args.vblora_vector_length,
                    topk=finetuning_args.vblora_topk,
                    target_modules=target_modules,
                    vblora_dropout=finetuning_args.vblora_dropout,
                    init_vector_bank_bound=finetuning_args.vblora_init_vector_bank_bound,
                    init_logits_std=finetuning_args.vblora_init_logits_std,
                    save_only_topk_weights=finetuning_args.vblora_save_only_topk_weights,
                    modules_to_save=finetuning_args.additional_target,
                    task_type=TaskType.CAUSAL_LM,
                )
            elif finetuning_args.pissa_init:
