...
class LoraArguments:
    ...
    use_randlora: bool = field(
        default=False,
        metadata={"help": "Whether or not to use RandLoRA (full-rank parameter-efficient fine-tuning)."},
    )
    randlora_prng_key: int = field(
        default=8539,
        metadata={"help": "The PRNG key for generating random projections in RandLoRA."},
    )
...
@dataclass
class VBLoRAArguments:
    r"""Arguments pertaining to the VB-LoRA training."""

    use_vblora: bool = field(
        default=False,
        metadata={"help": "Whether or not to use VB-LoRA (Vector Bank Low-Rank Adaptation)."},
    )
    vblora_num_vectors: int = field(
        default=256,
        metadata={"help": "Number of vectors in the vector bank. Use higher values when the model size increases."},
    )
    vblora_vector_length: int = field(
        default=256,
        metadata={
            "help": (
                "The length of the vectors in the vector bank. "
                "The length of the vectors should be divisible by the hidden dimension of the model."
            )
        },
    )
    vblora_topk: int = field(
        default=2,
        metadata={
            "help": (
                "The K value for top-K selection. A larger value of K increases the size of the saved model. "
                "In practice, setting K=2 typically provides the best performance and parameter efficiency."
            )
        },
    )
    vblora_dropout: float = field(
        default=0.0,
        metadata={"help": "The dropout probability for VB-LoRA layers."},
    )
    vblora_init_vector_bank_bound: float = field(
        default=0.02,
        metadata={
            "help": (
                "The vector bank is initialized with a uniform distribution between -init_vector_bank_bound and "
                "init_vector_bank_bound. A small value, such as 0.02, is typically effective."
            )
        },
    )
    vblora_init_logits_std: float = field(
        default=0.1,
        metadata={
            "help": (
    ...
    vblora_save_only_topk_weights: bool = field(
        default=False,
        metadata={
            "help": (
                "Whether to only save the topk weights. Setting this to True significantly reduces storage space. "
                "However, models saved in this mode can be used for merging or inference only, not for resuming training."
            )
        },
    )
...
class FinetuningArguments(
    ...
    def __post_init__(self):
        ...
        if self.finetuning_type != "lora":
            ...
            if self.use_randlora:
                raise ValueError("`use_randlora` is only valid for LoRA training.")

            if self.pissa_init:
                raise ValueError("`pissa_init` is only valid for LoRA training.")

        if self.use_randlora and self.use_dora:
            raise ValueError("Cannot use RandLoRA with DoRA together.")

        if self.use_randlora and self.pissa_init:
            raise ValueError("Cannot use RandLoRA with PiSSA together.")

        if self.use_vblora:
            if self.finetuning_type != "lora":
                raise ValueError("`use_vblora` is only valid for LoRA training.")

            if self.use_dora:
                raise ValueError("Cannot use VB-LoRA with DoRA together.")

            if self.use_randlora:
                raise ValueError("Cannot use VB-LoRA with RandLoRA together.")

            if self.pissa_init:
                raise ValueError("Cannot use VB-LoRA with PiSSA together.")
